...
describe 'Waybill API' do
  ...
  describe 'PATCH /update' do
    ...
    context 'unauth' do
      it 'returns 401 status if there is no access_token' do
        patch "/api/v1/api_waybills/#{waybill.waybill_id}", format: :json
        expect(response.status).to eq 401
      end

      it 'returns 401 status if access token is invalid' do
        patch "/api/v1/api_waybills/#{waybill.waybill_id}", format: :json, access_token: '1234'
        expect(response.status).to eq 401
      end
    end
    ...
    context 'auth' do
      ...
      it 'returns 200 status' do
        patch "/api/v1/api_waybills/#{waybill.waybill_id}", waybill: attributes_for(:waybill), format: :json, access_token: access_token.token
        expect(response).to be_success
      end
      ...
      it 'changes waybill attributes' do
        patch "/api/v1/api_waybills/#{waybill.waybill_id}", waybill: { fio: waybill_updated_fio }, format: :json, access_token: access_token.token
        waybill.reload
        expect(waybill.fio).to eq waybill_updated_fio
      end

      it 'does not change record count in the database' do
        expect { patch "/api/v1/api_waybills/#{waybill.waybill_id}", waybill: { fio: waybill_updated_fio }, format: :json, access_token: access_token.token }.to_not change(Waybill, :count)
      end
